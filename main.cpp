...
//#include <string>
#include <thread>
#include "jj_http_message.h"
#include "jj_http_server.h"
#include "jj_uri.h"
...
int sa_Age = 0;
int sa_New = 0;
int sa_AvailTrackedNew = 0;
int sa_Banned = 0;
int sa_sys_cpu = 0;
int sa_sys_rss = 0;
int sa_sys_vsz = 0;
uint64_t sa_requests = 0;
uint64_t sa_queries = 0;
...
bool isNumber(const string& s)
{
  ...
  CDnsSeedOpts() : nThreads(96), nDnsThreads(4), ip_addr("::"), nPort(53), nWebPort(0), mbox(NULL), ns(NULL), host(NULL), tor(NULL), fUseTestNet(false), fWipeBan(false), fWipeIgnore(false), ipv4_proxy(NULL), ipv6_proxy(NULL) {}
  ...
  void ParseCommandLine(int argc, char **argv) {
    ...
    std::string e_port = reader.Get("server", "Port", "0");
    std::string e_webport = reader.Get("server", "Webport", "0");
    std::string e_testnet = reader.Get("server", "Testnet", "0");

    char* jj_ns = strcpy(new char[e_ns.length() + 1], e_ns.c_str()); 
    char* jj_ho = strcpy(new char[e_ho.length() + 1], e_ho.c_str()); 
    char* jj_ip = strcpy(new char[e_ip.length() + 1], e_ip.c_str()); 
    char* jj_mail = strcpy(new char[e_mail.length() + 1], e_mail.c_str()); 
    if(isNumber(e_port)) { jj_port = std::stoi(e_port); }else{}
    if(isNumber(e_webport)) { jj_webport = std::stoi(e_webport); }else{}
    if(isNumber(e_testnet)) { jj_testnet = std::stoi(e_testnet); }else{}
    if(jj_testnet == 1){ fUseTestNet = 1; }
    ...
    while(1) {
      ...
      int option_index = 0;
      int c = getopt_long(argc, argv, "h:n:m:t:a:p:d:o:i:k:w:s:", long_options, &option_index);
      if (c == -1) break;
      switch (c) {
        case 'h': {
          host = optarg;
    ...
    if (ns == NULL){ ns = jj_ns; }
    if (host == NULL){ host = jj_ho; }
    if (ip_addr == NULL){ ip_addr = jj_ip; }
    if (mbox == NULL){ mbox = jj_mail; }
    if (nPort > 0 && nPort < 65536){ /*--*/ } else if (jj_port > 0 && jj_port < 65536) { nPort = jj_port; }
    if (nWebPort > 0 && nWebPort < 65536){ /*--*/ } else if (jj_webport > 0 && jj_webport < 65536) { nWebPort = jj_webport; }
...
extern "C" void* ThreadStats(void*) {
  ...
  do {
    ...
    sa_sys_vsz = getmemvsz();
    sa_sys_cpu = getcpu();
    printf("%s %i/%i available (%i tried in %is, %i new, %i active), %i banned; %llu DNS requests, %llu db queries RSS %i MB / CPU %i ", c, stats.nGood, stats.nAvail, stats.nTracked, stats.nAge, stats.nNew, stats.nAvail - stats.nTracked - stats.nNew, stats.nBanned, (unsigned long long)requests, (unsigned long long)queries, sa_sys_rss, sa_sys_cpu);
    
    sa_Good = stats.nGood;
    sa_Avail = stats.nAvail;
    sa_Tracked = stats.nTracked;
    sa_Age = stats.nAge;
    sa_New = stats.nNew;
    sa_AvailTrackedNew = stats.nAvail - stats.nTracked - stats.nNew;
    sa_Banned = stats.nBanned;
...
static const string *testseeds = testnet_seeds;
...
int main(int argc, char **argv) {
  ...
  if (opts.nWebPort) {
    try {
      server.Start();
      std::cout << "Webserver listening on port " << opts.nWebPort << std::endl;
  ...
  server.RegisterHttpRequestHandler("/", HttpMethod::HEAD, say_index);
  server.RegisterHttpRequestHandler("/", HttpMethod::GET, say_index);
  server.RegisterHttpRequestHandler("/nodes.html", HttpMethod::HEAD, send_nodes);
  server.RegisterHttpRequestHandler("/nodes.html", HttpMethod::GET, send_nodes);
  server.RegisterHttpRequestHandler("/seeds.html", HttpMethod::HEAD, send_seeds);
  server.RegisterHttpRequestHandler("/seeds.html", HttpMethod::GET, send_seeds);
  server.RegisterHttpRequestHandler("/system.html", HttpMethod::HEAD, send_system);
  server.RegisterHttpRequestHandler("/system.html", HttpMethod::GET, send_system);
  server.RegisterHttpRequestHandler("/statistics.html", HttpMethod::HEAD, send_statistics);
  server.RegisterHttpRequestHandler("/statistics.html", HttpMethod::GET, send_statistics);
